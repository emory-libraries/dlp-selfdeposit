...
RSpec.describe "viewing a FileSet's show page", :clean_repo, :perform_enqueued, type: :feature do
  ...
  before do
    publication
    pulled_file_set = Hyrax.query_service.find_by(id: publication.member_ids.first)
    Hyrax.index_adapter.save(resource: pulled_file_set)
  ...
  it 'contains the custom characterization elements' do
    ...
    expect(page).to have_content('Persistent Unique Identification: fmt')
  ...
  it 'contains a table of Preservation Events' do
    ...
    # NOTE: the below test is randomly failing, most likely to the page loading before PreservationEvents are fully persisted.
    expect(find_all('table#fs-preservation-event-table tbody tr')).to be_present # expect at least one body row
  end
end
