...
RSpec.describe CatalogController, type: :controller do
  ...
  describe 'default_solr_params' do
    let(:pulled_solr_fields) { controller.blacklight_config.default_solr_params[:qf].split(' ') }

    it { expect(pulled_solr_fields).to match_array(default_search_fields) }
  end
  ...
  context 'targeted search' do
    ...
    describe 'Default Search' do
      default_search_fields.reject { |v| v == 'id' }.each do |field|
        before do
          Hyrax.index_adapter.wipe!
          bingo.send("#{field.split('_')[0..-2].join('_')}=", "Bingo")
          Hyrax.persister.save(resource: bingo)
        ...
        it 'finds works with the given search term' do
          get :index, params: { q: 'bingo', search_field: 'search' }
          expect(response).to be_successful
          expect(response).to render_template('catalog/index')
          expect(assigns(:response).documents.map(&:id)).to contain_exactly(bingo.id)
        end
