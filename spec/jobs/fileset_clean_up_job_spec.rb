...
RSpec.describe FilesetCleanUpJob, :clean do
  ...
  shared_examples 'file set characterization' do
    it 'queues characterization job and logs to CSV' do
      expect(ValkyrieCharacterizationJob).to have_received(:perform_later).with(file_set.file_ids.first).at_least(:once)
  ...
  context 'when no file_set_ids are provided' do
    ...
    it 'processes all file sets' do
      ...
      expect(ValkyrieCharacterizationJob).to have_received(:perform_later).at_least(:once)
    end
  ...
  context 'when specific file_set_ids are provided' do
    ...
    before do
      allow(query_service).to receive(:find_by).and_return(file_set1, file_set2)
