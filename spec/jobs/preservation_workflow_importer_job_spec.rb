...
RSpec.describe PreservationWorkflowImporterJob, :clean do
  let(:generic_work) { GenericWork.where(deduplication_key: 'MSS1218_B071_I205').first }
  let(:csv)          { fixture_path + '/preservation_workflows.csv' }

  before do
    GenericWork.create(title: ['Example title'], deduplication_key: 'MSS1218_B071_I205')
  end
